...
const WEBHOOK_PATH='instagram-billing-status-sync';
const WEBHOOK_ID='31d635a3-6d4d-4c65-8ca9-51b89ec6fac5';
...
if(String(input.action||'').toLowerCase()==='health'){
  return [{json:{ok:true,service:'instagram-billing-status-sync',status:'healthy',timestamp:now}}];
}
...
const log=(stage,data={})=>{try{console.log('[IG_BILLING_STATUS] '+JSON.stringify({ts:new Date().toISOString(),stage,...data}));}catch{}};
...
for(const p of templatePages){
  const key=title(p.properties?.Estado).trim();
  if(!key) continue;
  templates.set(key,{message:rich(p.properties?.['Mensaje Instagram']),active:!!p.properties?.Activo?.checkbox,type:p.properties?.['Tipo evento']?.select?.name||''});
}

const summary={scanned:orders.length,changed:0,sent:0,disabled:0,failed:0,profilesUpdated:0,skippedCooldown:0};
for(const order of orders){
  const props=order.properties||{};
  const state=String(props['Estado facturación']?.select?.name||'').trim();
  const last=rich(props['Último estado facturación notificado']).trim();
  if(!state||state===last) continue;
  summary.changed++;

  const lastError=rich(props['Error última notificación facturación']);
  const lastAttempt=props['Fecha última notificación facturación']?.date?.start||'';
  ...
  const templateKey='Facturación · '+state;
  const tpl=templates.get(templateKey);
  if(!tpl){
    const e='['+state+'] No existe la plantilla '+templateKey+' en Mensajes por estado.';
    await patchOrder(order.id,{'Error última notificación facturación':textProp(e),'Fecha última notificación facturación':{date:{start:now}}});
    summary.failed++;log('TEMPLATE_MISSING',{orderId:order.id,state,templateKey});continue;
  }
  if(!tpl.active){
    await patchOrder(order.id,{'Último estado facturación notificado':textProp(state),'Error última notificación facturación':textProp('')});
    summary.disabled++;log('NOTIFICATION_DISABLED',{orderId:order.id,state,templateKey});continue;
  }
  if(!igsid){
  ...
  const vars={
    ...
    estado:state,
    estado_facturacion:state,
    saldo_cobrado:String(props['Saldo cobrado']?.number??''),
    saldo_pendiente:String(props['Saldo pendiente']?.number??''),
    metodo_pago:String(props['Método de pago']?.select?.name||'')
  };
  ...
  try{
    ...
    await patchOrder(order.id,{
      'Último estado facturación notificado':textProp(state),
      'Fecha última notificación facturación':{date:{start:now}},
      'Error última notificación facturación':textProp('')
    });
    summary.sent++;
    log('NOTIFICATION_SENT',{orderId:order.id,orderNumber,state,templateKey,igsid,messageId:sent?.message_id||null});
  }catch(e){
    const msg='['+state+'] '+errText(e);
    await patchOrder(order.id,{'Error última notificación facturación':textProp(msg),'Fecha última notificación facturación':{date:{start:now}}});
    summary.failed++;
    log('NOTIFICATION_FAILED',{orderId:order.id,orderNumber,state,templateKey,igsid,error:errText(e)});
  }
}
...
async function main(){
  if(!N8N_URL||!N8N_API_KEY){console.log('[IG_BILLING_STATUS_INSTALL] skipped: n8n config missing');return;}
  const nodes=[
    {id:'ig-billing-status-schedule',name:'Every Minute',type:'n8n-nodes-base.scheduleTrigger',typeVersion:1.2,position:[0,-120],parameters:{rule:{interval:[{field:'cronExpression',expression:'0 * * * * *'}]}}},
    {id:'ig-billing-status-webhook',name:'Manual Billing Sync',type:'n8n-nodes-base.webhook',typeVersion:2,position:[0,120],webhookId:WEBHOOK_ID,parameters:{httpMethod:'POST',path:WEBHOOK_PATH,responseMode:'lastNode',options:{}}},
    {id:'ig-billing-status-code',name:'Notify Instagram Billing Status',type:'n8n-nodes-base.code',typeVersion:2,position:[300,0],parameters:{mode:'runOnceForAllItems',jsCode:notifierCode}}
  ];
  const connections={
    'Every Minute':{main:[[{node:'Notify Instagram Billing Status',type:'main',index:0}]]},
    'Manual Billing Sync':{main:[[{node:'Notify Instagram Billing Status',type:'main',index:0}]]}
  };
  ...
  if(existing){
    ...
    console.log('[IG_BILLING_STATUS_INSTALL] updated '+id);
  }else{
    const created=await n8n('/workflows',{method:'POST',body:JSON.stringify({name:WORKFLOW_NAME,nodes,connections,settings})});
    id=created.id;
    console.log('[IG_BILLING_STATUS_INSTALL] created '+id);
  }
  await n8n('/workflows/'+encodeURIComponent(id)+'/activate',{method:'POST'});
  console.log('[IG_BILLING_STATUS_INSTALL] active '+JSON.stringify({workflowId:id,webhookPath:WEBHOOK_PATH}));
  try{
    ...
    const t=await r.text();
    console.log('[IG_BILLING_STATUS_INSTALL] smoke '+JSON.stringify({status:r.status,body:t.slice(0,1000)}));
  }catch(e){console.log('[IG_BILLING_STATUS_INSTALL] smoke non-fatal '+String(e?.message||e));}
}

main().catch(e=>{console.error('[IG_BILLING_STATUS_INSTALL] failure: '+String(e?.stack||e?.message||e));process.exitCode=1;});
